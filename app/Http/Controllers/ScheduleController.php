...
use App\Services\CourseHallAssigner; // Your new optimized service
...
class ScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // We match the time slots used in the generator logic.
    ...
    // Removed HallAssigner from the parameters here
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->type === 'course') {
                // For courses, trigger the dedicated CourseHallAssigner AFTER the schedule is made
                $adminAlerts = $this->courseHallAssigner->assignHallsToCourseSchedule($schedule->id);
            } else {
                // For exams, ExamHallService runs INSIDE ScheduleGenerator, so we just grab the alerts it already made
                $adminAlerts = $result['admin_alerts'] ?? [];
            }

            // Reload relationships to ensure the new hall mappings are attached to the response
            ...
            foreach ($usersToNotify as $notificationUser) {
                $notificationUser->notify(new SchoolNotification(
